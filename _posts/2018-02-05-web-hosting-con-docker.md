...
si no es así, en Wikipedia [hay info][1] :) No es mas que una forma de emular algo
usando software, donde ese _algo_ es un ordenador, es hardware. Otras formas de virtualizar
seria, la de los contenedores. Lo que tienes real es el hardware, y el sistema operativo,
pero lo demás (archivos, programas, redes...) queda completamente aislado. El software
en un contenedor solo ve lo que viene en su ISO y lo que tu decidas compartir con él.
...
también algunas plataformas Cloud [ofrecen][4] ejecución de contenedores Docker o
[parecidos][5]. En muchas empresas, se está usando la combinación de máquinas virtuales
(en servidores especializados), con contenedores para realizar un flujo de trabajo
moderno, fluido y fácilmente mantenible y escalable (ahora entraremos en terminos
especificos). Y no hablo solo de empresas de desarrollo software, seguramente
habrá empresas que incluyan este tipo de sistemas para agilizar algunas partes de su
flujo de trabajo.
...
y añadido. Eso permite ahorrar espacio aunque el rendimiento verse reducido. Un contendor
al fin y al cabo es una imagen en ejecución, con los parámetros que se le haya especificado.
Por ultimo, uno siempre puede persistir (dejar persitente cambios realizados en el contenedor)
...
solo serias capaz de ver aquellas que tu publicaras. Aunque tambien está la versión de
...
servidores distribuidos (cluster) y que crean que solo hay un ordenador y todos están ahí.
...
al dominio (o dominios separados por coma `,`).
...
de calidad) para cualquier web que, además de cumplir lo anterior, añada dos variables de
...
uso `docker-compose`, para el resto de contenedores, la red es `nginxproxy_dbnet`.
...
services:
  ...
  mariadb:
    ...
    volumes:
      - /root/docker/_mysql:/var/lib/mysql
    networks:
      - dbnet
  ...
  phpmyadmin:
    ...
    restart: always
    environment:
      - VIRTUAL_HOST=phpmyadmin.recaweb.local
      - PMA_ARBITRARY=1
      - "PMA_ABSOLUTE_URI=http://phpmyadmin.recaweb.local"
      - MYSQL_ROOT_PASSWORD=UNA-CONTRASEÑA
      - PMA_HOST=mariadb
    networks:
      - dbnet
...
Vosotros podeis montar el servidor como queráis, usando PHP7 FPM con nginx o con Apache, o lo
que necesitéis. Es muy fácil configurar imágenes para usarlas en la web :)

## Web como contenedor

Ahora es cuando podemos empezar a poner webs nuevas. En mi caso, sigo los siguientes pasos:

 1. Creo una carpeta donde almacenar todo
...
Y, si en lugar de esto, usas contenedores con aplicaciones listas (como [Jenkins][16]),
...
Para ahorraros tiempo, si usais un Wordpress con este sistema y no usáis la imagen de Docker
de Wordpress, teneis que configurar una cosa para que no tengas un bucle infinito de redirecciones.
