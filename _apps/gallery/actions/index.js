...
export const loadFirstPhotos = (user_id, photoset_id, per_page, page) => dispatch => {
    flickr.photosets.getPhotos({ user_id, photoset_id, per_page, page }, json => {
        flickr.photos.getSizes({ photo_id: json.photoset.primary }, son => {
...
export const loadMorePhotos = (user_id, photoset_id, per_page, page, next = false) => dispatch => {
    ...
    flickr.photosets.getPhotos({ user_id, photoset_id, per_page, page: page + 1 }, json => {
...
export const loadDetailedPhoto = photo => dispatch => {
    ...
    if(cache.info) {
        ...
        flickr.photos.getInfo({ photo_id: photo.id, secret: photo.secret }, json => {
    ...
    if(cache.exif) {
        ...
        flickr.photos.getExif({ photo_id: photo.id, secret: photo.secret }, json => {
...
export const enablePhotoZoom = photo => dispatch => {
    if(photo.zoomUrl === undefined) {
        flickr.photos.getSizes({ photo_id: photo.id }, json => {
            let size = null;
